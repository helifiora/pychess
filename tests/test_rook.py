...
from pychess.piece import Rook
...
@mark.parametrize('entry, output', [
    ...
    (
            ...
            [
                ...
                Position(0, 5), Position(0, 6), Position(0, 7),
    ...
    (
            Position(4, 7),
            [
                Position(4, 6), Position(4, 5), Position(4, 4), Position(4, 3), Position(4, 2), Position(4, 1),
                Position(4, 0),
                Position(3, 7), Position(2, 7), Position(1, 7), Position(0, 7),
    ...
    (
            Position(7, 4),
            [
                Position(7, 3), Position(7, 2), Position(7, 1), Position(7, 0),
                Position(7, 5), Position(7, 6), Position(7, 7),
                Position(6, 4), Position(5, 4), Position(4, 4), Position(3, 4), Position(2, 4),
                Position(1, 4), Position(0, 4)
            ]
    ),
])
def test_moviments_when_rook_in_middle_corner_position(entry: Position, output: list[Position]):
    board = Board.empty()
    rook = Rook(Color.BLACK)
    board.place(rook, entry)
    assert set(rook.movements()) == set(output)
...
def test_moviments_with_piece_and_ally_pieces_in_board(entry: Position, other: list[Position], output: list[Position]):
    ...
    for p in other:
        board.place(Rook(Color.WHITE), p)
